...
void PathfindingAlgorithm::printMap(){
    for(int i=0; i<11; i++){
        for(int j=0; j<20; j++){
            cout<<Map[i][j] << " ";
        }
...
int PathfindingAlgorithm::pathFinding (int xS, int yS, int xF, int yF, int gameMap[20][11]){
    for(int i=0; i<11; i++){
        for(int j=0; j<20; j++){
            Map[i][j] = gameMap[i][j];
        }
    ...
    int nx=xS+dx[atoi(&(route.at(0)))];
    int ny=yS+dy[atoi(&(route.at(0)))];
    if(nx<xS){
        if(ny<yS){
            return 1;
        }else if(ny==yS){
